...
async def guard_url(url):
    ...
    for x in socket.getaddrinfo(host, u.port or (443 if u.scheme == 'https' else 80)):
        ...
        if not settings().allow_private_targets and (ip.is_private or ip.is_loopback or ip.is_link_local): 
            raise ValueError('Private/loopback targets are disabled')

async def discover(url, mode):
    await guard_url(url)
    if mode == 'openapi': 
        return await discover_openapi(url)
    if mode == 'browser': 
        return await discover_browser(url)
            ...
            try:
                ...
                if 'paths' in d:
                    return {
                        ...
                        'ready': True,
                        'spec_url': x,
                        'operations': [{'path': p, 'method': m, 'summary': o.get('summary', '')} 
                                       for p, v in d['paths'].items() 
                                       for m, o in v.items() if m.lower() in {'get', 'post', 'put', 'patch'}]
                    }
            except Exception: 
                pass
...
async def _bypass_turnstile_if_present(page):
    ...
    try:
        # Wait briefly to see if Turnstile / Cloudflare iframe appears
        for frame in page.frames:
            if "challenges.cloudflare.com" in frame.url or "turnstile" in frame.url:
                ...
                if await checkbox.count():
                    ...
                    return True
        # Check shadow DOM / main page challenge
...
async def discover_browser(url):
    async with async_playwright() as p:
        ...
        # --- BYPASS CLOUDFLARE/CAPTCHA IF PRESENT ---
        ...
        data = await page.evaluate("""() => { 
            ...
            const pick = s => [...document.querySelectorAll(s)].filter(vis).map(e => ({
            ...
            return {
                ...
                inputs: pick('textarea,input,[contenteditable=true]'),
        ...
        await b.close()
        likely = bool(data['inputs'] and data['buttons'])
        ...
        return {
            ...
            'recommended': {
                ...
                'response_selector': 'main, [role=log], [aria-live=polite]'
            },
            'notes': [] if likely else ['No obvious chat input and submit control found. Supply selectors or use transcript mode.']
        }