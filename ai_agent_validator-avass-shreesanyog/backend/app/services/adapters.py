...
Three modes are supported end to end:
  * rest / openapi -> RestAdapter, for any live FastAPI or standard REST agent
  * browser        -> BrowserAdapter, for a hosted chat UI via Playwright
  * transcript     -> TranscriptAdapter, for replayed/uploaded conversations

Every adapter returns the same envelope so the evaluation tiers stay
adapter-agnostic:
    {'text': str, 'latency_ms': float, 'evidence': dict}

Multi-turn is a first-class concern: `invoke_case` walks a list of turns,
carrying conversation state forward (session id for REST, same page for
browser), so a `multi_turn` test case exercises real dialogue rather than a
...
# Response keys commonly used by chat/agent endpoints, tried in order when the
# tenant has not configured an explicit response_path.
COMMON_RESPONSE_KEYS = ('response', 'message', 'answer', 'output', 'text', 'reply', 'result', 'content')


def _auth_headers(target) -> dict:
    """Decrypt stored target credentials into request headers. Never logged."""
...
def _extract(payload, response_path: str | None):
    """Pull the agent's reply out of a JSON body.

    `response_path` is a dotted path (e.g. 'data.choices.0.message.content').
    Without one, well-known keys are tried before falling back to raw JSON, so
    an unknown schema degrades to 'evaluate the whole body' rather than ''.
    """
    if response_path:
        cur = payload
...
class RestAdapter:
    """Drives any live REST/FastAPI agent endpoint.

    Configurable entirely through Target.config so no target-specific code is
    needed: method, path, prompt_field, response_path, session_field, headers.
    """
    ...
    async def invoke_case(self, target, case):
        ...
        async def _send(client, method, url, body, headers):
            """One request with bounded retry + exponential backoff on transient
            failures (network errors and 429/5xx). 4xx (except 429) is not retried
            — it's the agent's answer, not a transport blip."""
            last_exc = None
        ...
        async with httpx.AsyncClient(timeout=cfg.get('timeout', 60), follow_redirects=True) as client:
            for turn in turns:
                ...
                # Trace collection: capture tool-call evidence when the agent exposes it.
                if isinstance(payload, dict):
...
class BrowserAdapter:
    """Drives a hosted chat UI with Playwright, holding one page across turns."""
    ...
    async def invoke_case(self, target, case):
        ...
        async with async_playwright() as p:
            b = await p.chromium.launch(headless=settings().browser_headless)
            page = await b.new_page()
            errs = []
            page.on('pageerror', lambda e: errs.append(str(e)))
            await page.goto(target.base_url, wait_until='domcontentloaded', timeout=settings().browser_timeout_ms)

            # Dynamic wait: pause until the SPA has actually rendered the chat box.
            try:
            ...
            for turn in turns:
                ...
                # Execution wait: allow the target agent to stream its reply.
                try:
                    await page.wait_for_function(
                        '(x)=>{const e=document.querySelector(x.s);return e && e.innerText.trim()!==x.b.trim()}',
                        arg={'s': response, 'b': before}, timeout=60000)
                except Exception:
