...
class Settings(BaseSettings):
    ...
    composite_quality_weight: float = 0.0  # Tier-2 generic quality (DeepEval); 0 keeps legacy behaviour
    ...
    # Async runner resilience (Phase 2)
    request_max_retries: int = 2
    request_backoff_seconds: float = 0.5
    request_rate_limit_per_sec: float = 0.0  # 0 disables client-side rate limiting

    # Optional: skip create_all when Alembic manages the schema
